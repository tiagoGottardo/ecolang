...
function Timer:set(timerTable)
  ...
  self.startTime=nil
  self.label = timerTable.label or self.label or secondsToTimeFormat(self.timerDuration)
...
function Timer:start(timerDuration)
  ...
  self.label=secondsToTimeFormat(self.timerDuration)
end

function Timer:update()
  local timePassed=love.timer.getTime()-self.startTime
  self.label=secondsToTimeFormat(math.max(self.timerDuration-timePassed, 0))
end
